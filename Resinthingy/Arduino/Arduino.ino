//#include <MultiStepper.h> your gay
...
float StepPLen;
float LenPers;
float ExtrudeMotorSpeed;
float OtherMotorSpeed;
float Muscle_Length;
float NumCoils;
float coilAngle;
...
void setup() {
  ...
  // Start_Group.addStepper(ColdDrawStepper)
  // Start_Group.addStepper
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    LenPers = getValue(msg, ',', 2).toFloat();
    pitch = getValue(msg, ',', 3).toFloat();
    CalLen = getValue(msg, ',', 1).toFloat();

    StepPLen = CalSteps/CalLen;
    ExtrudeMotorSpeed = StepPLen * LenPers;
    OtherMotorSpeed = ((stepsPerRevolution *70/59) / pitch) * LenPers;
    ...
    if (CommandString.equals("A")) {
      ...
      buttonMode = 3;  // extrude
    } else if (CommandString.equals("D")) {
      buttonMode = 4;  // Calibration
      ColdDrawStepper.move(CalSteps);
  ...
  if (buttonMode == 2) {  //stop
    ...
    ColdDrawStepper.setMaxSpeed(2000);
    UpperMandrelStepper.setMaxSpeed(2000);

    ColdDrawStepper.setSpeed(ExtrudeMotorSpeed);
    UpperMandrelStepper.setSpeed(OtherMotorSpeed);
    ColdDrawStepper.runSpeed();
    UpperMandrelStepper.runSpeed();
  }

  else if (buttonMode == 3) {  //Extrude
  ...
  } else if (buttonMode == 4) {  // Calibration

    ColdDrawStepper.setMaxSpeed(1000);
    ColdDrawStepper.setSpeed(500);
    // float time = 10000;  //s 
    // ColdDrawStepper.runSpeed();
    // if (millis() - motionstarttime >= time) {
    //   ColdDrawStepper.stop();
    //   buttonMode = 2;
    // }

  if (ColdDrawStepper.distanceToGo() != 0) {
    ColdDrawStepper.run();  // Non-blocking step
  }
    // ColdDrawStepper.runToPosition();
    // buttonMode == 2;
  }
}
